...
window.onload = () => {
    $('#nav').load('/template/nav.html', () => {
        ...
        if (mb_id_session.length > 0 && mb_id_session !== null && mb_id_session !== "") {
            ...
            document.getElementsByClassName('navbar-nav')[0].children[2].innerHTML = "<li class='nav-item'><a class='nav-link' href='#'> ID : ["+mb_id_session+"]</a></li>";
            document.getElementsByClassName('navbar-nav')[0].children[3].innerHTML = "<li class='nav-item'><a class='nav-link' href='/member/logout'>로그아웃</a></li>";
        } else {
